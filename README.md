[![pipeline status](https://gitlab.com/aruw/controls/otu-mcb/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/otu-mcb/-/commits/develop)
...
```
cd my-amazing-project/otu-mcb-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `otu-mcb` (**not
   `otu-mcb-project`**)
...
1. Open the folder `aruw-otu-mcb-project` in VSCode. Hit the debug tab on the left side or type
...
`/otu-mcb-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/otu-mcb-project` (where the
